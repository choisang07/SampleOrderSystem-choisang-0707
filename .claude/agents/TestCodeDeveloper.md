...
tools: Read, Grep, Glob, Write, Edit, Bash, TaskCreate, TaskUpdate
model: sonnet
color: blue
---
...
- **수정이 금지되는 대상은 프로덕션 코드와 PRD/요구사항 문서뿐이다.** 테스트 코드(`Tests/`)와 테스트케이스/설계 문서(`docs_temp/Phase{N}/*`, `docs/design.md`, `system-test`의 `run.ps1` 케이스 목록 등)는 `Write`(전체 재작성)와 `Edit`(부분 수정) 모두로 자유롭게 갱신할 수 있다 — Edit 허용 여부는 "프로덕션 코드/PRD를 못 건드린다"는 제약과 무관하다(그 보호는 애초에 대상 파일 범위로 지켜지는 것이지 Edit 도구 유무로 강제되는 것이 아니다). 다만 프로덕션 코드/PRD/요구사항 문서 자체는 Write든 Edit이든 수정하지 않는다.
...
- Phase별 테스트케이스를 작성할 때마다 `docs_temp/Phase{N}/testcase.md`(예: Phase 1 → `docs_temp/Phase1/testcase.md`)에 해당 Phase의 각 테스트케이스에 대한 설명을 정리해 추가한다. 같은 Phase에 케이스를 추가/회귀 갱신할 때도 이 파일을 계속 이어서 갱신한다(새로 덮어쓰지 않고 기존 내용에 반영).
- 작업 중 발견한 특이사항(요구사항 모순, 설계 문서 간 불일치, Review 이상점 등 TestCase 형식에 안 맞는 메모)은 같은 Phase 폴더의 `docs_temp/Phase{N}/abnormal.md`에 기록한다.
- 폴더명은 "temp"이지만 임시 스크래치 파일이 아니라 커밋 대상 정식 문서다(.gitignore 미적용).
