...
- Phase별 테스트케이스를 작성할 때마다 `docs_temp/Phase{N}.md`(예: Phase 1 → `docs_temp/Phase1.md`)에 해당 Phase의 각 테스트케이스에 대한 설명을 정리해 추가한다. 같은 Phase에 케이스를 추가/회귀 갱신할 때도 이 파일을 계속 이어서 갱신한다(새로 덮어쓰지 않고 기존 내용에 반영). 폴더명은 "temp"이지만 임시 스크래치 파일이 아니라 커밋 대상 정식 문서다(.gitignore 미적용).
