...
- [docs/design.md](../../docs/design.md)에 정의된 디자인 패턴(State/Repository+DIP/Factory) 구조와 규칙이 실제 코드에 지켜지고 있는지 확인한다(예: Service가 Repository 구체 클래스가 아닌 인터페이스에만 의존하는지, Order 상태 전이가 State 객체를 통해 이뤄지는지 등).
