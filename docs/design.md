...
ID 포맷은 [docs/screens.md](screens.md)의 화면 예시를 근거로 시료 `S-{3자리 순번}`, 주문 `ORD-{YYYYMMDD}-{HHMMSS}`를 현재 기준으로 삼는다(샘플 데이터 기준이라 최종 확정은 아니며, `SampleFactory`/`OrderFactory` 구현 시점에 재확인).

```json
{
  "samples": [
    { "id": "S-001", "name": "WaferA", "avgProductionTime": 2.0, "yield": 0.9, "stock": 5 }
  ],
  "orders": [
    { "id": "ORD-20260715-090000", "sampleId": "S-001", "customerName": "홍길동", "quantity": 3, "status": "RESERVED", "createdAt": "2026-07-15 09:00:00" }
  ],
  "productionQueue": [
    { "orderId": "ORD-20260715-110000", "sampleId": "S-002", "requiredQuantity": 13, "totalProductionMinutes": 45.5, "startedAt": "2026-07-15 11:00:00" }
  ]
}
```
...
| 파일 손상/스키마 불일치 시 무언의 실패 (DataMonitor) | `JsonFileStore::load`(`isCorrupted()`) 실패 시 `false`만 반환하는 대신, **`main.cpp`가 그 결과를 확인해 `bool` 플래그 하나로 `MainController`에 전달**하고, `MainController`는 그 플래그가 `true`면 시작 시 "데이터 파일 로드 실패" 배너를 View에 출력하도록 계약 변경. `MainController`는 `JsonFileStore`(구체 Persistence 타입)를 직접 참조하지 않는다 — §3의 DIP 원칙(Controller는 인터페이스/원시 타입만 안다)을 지키기 위한 것으로, Phase 0 리뷰에서 `MainController`가 `JsonFileStore&`를 직접 들고 있던 것이 이 원칙 위반으로 지적되어 확정한 방식이다. |
...
| ID 채번 방식(접미사 파싱)의 한계 (DummyDataGenerator) | 본 프로젝트는 화면 예시([docs/screens.md](screens.md))를 근거로 `S-{순번}`/`ORD-{날짜}-{시각}` 포맷을 현재 기준으로 삼는다(§5). PoC의 단순 접미사 파싱 방식은 그대로 쓰지 않되, "Repository 구현체가 채번을 전담해 Service/Domain 계층이 ID 형식을 직접 알 필요 없게 캡슐화"하는 원칙은 유지 — `SampleFactory`/`OrderFactory`가 실제 채번 로직을 담당 |
...
- **빌드 구성 (Phase 0에서 확정, 별도 Tests 프로젝트를 두지 않음)**: GoogleTest/GoogleMock은 `packages.config` + NuGet 패키지(`gmock`, v1.11.0)로 받는다(`SampleOrderSystem.vcxproj`가 `..\packages\gmock.1.11.0\build\native\gmock.targets`를 임포트 — nlohmann/json처럼 단일 헤더로 직접 벤더링하지 않는다). 테스트 코드는 같은 `SampleOrderSystem` 프로젝트 안의 `Tests/` 폴더(예: `Tests/OrderServiceTest.cpp`, `Tests/Mocks/MockSampleRepository.h`)에 두고, 프로젝트 루트의 `main.cpp` 하나가 빌드 구성에 따라 진입점을 분기한다:
  ```cpp
